...
import JobGateService from '../services/job-gate.service';

class JobGateController {
  createJobGate = async (req: Request, res: Response) => {
    const createBy = res.locals.user;
    const blockList = res.locals.requestData;

    new CREATED({
      message: SUCCESS_MESSAGE.SAVE_BLOCK_SUCCESS,
      metadata: await JobGateService.createJobGate(blockList, createBy),
    }).send(res);
  };
...
export default new JobGateController();
