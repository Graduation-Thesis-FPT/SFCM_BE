...
class InvoiceManagementMisa {
    ...
    postToMS = async (path, data, moreConfig = {}) => {
        this.resetValue();
        ...
        try {
            ...
            if (this.responseData["ErrorCode"] && this.responseData["Success"] === false) {
                this.data.success = false;
                this.data["error"] = "[Misa] " + this.responseData["ErrorCode"] + " : " + (this.responseData.Errors[0] || this.getErrorMisa(this.responseData["ErrorCode"] || this.responseData.DescriptionErrorCode || 'Unknown error'));
                return false;
            }
            ...
            if (this.responseContent) {
                ...
                let errorMsg = "[Misa] " + (response["code"] || response["status"] || 200) + " - " + (response["data"] || response["error"] || response["message"] || "Thất bại: Giao dịch với Hệ Thống Hóa Đơn Điện Tử!");
    ...
    getToken = async () => {
        ...
        if (!this.responseData["Data"]) {
            this.data["error"] = `[Misa] ${this.responseData["title"]}: ${this.responseData["Errors"][0]}`;
            return false;
        }
    ...
    publish = async (req) => {
        ...
        var orderNo = args.orderNo || datas[0]["orderNo"] || await genOrderNo();
        ...
        for await (let [index, item] of datas.entries()) {
            if (typeof item === "object") {
                ...
                var unit = await this.getUnitName(item['UnitCode']);
                ...
                var unitPrice = parseFloat(urate * exchange_rate); //lam tron so luong+don gia theo yeu cau KT
                var amount = parseFloat(i_amt * exchange_rate);

                var taxPerText = item["VatRate"] ? parseFloat(String(item["VatRate"]).replace(",", "")) : "-2"; //-2 : Hoa dơn KCT
                var vat_amt = taxPerText == "-2" ? "" : parseFloat(String(item["Vat"]).replace(",", ""));
                var vat = taxPerText == "-2" ? "" : parseFloat(vat_amt * exchange_rate);
                switch (taxPerText) {
                    case 0:
                    case 5:
                    case 8:
        ...
        taxPerText === 'KCT' ? "" : invoice['OriginalInvoiceData']['TaxRateInfo'] = Object.values(TaxRateInfo);
    ...
    cancelInv = async (fkey, reason, cancelDate) => {
    ...
    cancelInvDirectly = async (req) => {
        ...
        var isSuccess = await this.postToMS(path, JSON.stringify({
    ...
    adjustInvoice = async (req) => {
        ...
        var vat_rate = datas[0]["VatRate"] ? parseFloat(datas[0]["VatRate"]) : "";
        var isCredit = args["isCredit"] || false;
        var orgInvNo = args["old_invNo"] || "";
        var adjust_type = args['adjust_type'] === 3 ? 1 : 2;
        var view_exchange_rate = "";
        ...
        var vat_amount = vat_amount * exchange_rate;
        var dvt = inv_type == "VND" ? " đồng" : " đô la Mỹ";
        var amount_in_words = FunctionModel.convert_number_to_words(total_amount) + dvt;
        amount_in_words = amount_in_words.charAt(0).toUpperCase() + amount_in_words.slice(1);

        var orderNo = await TaskModel.generateorderNo();
        var cusCode = String(cusTaxCode).trim();
        ...
        let invoice = {
            ...
            "OriginalInvoiceData": {
                //----Điều chỉnh/thay thế----
                "ReferenceType": adjust_type,
                "OrgInvoiceType": 3,
                "OrgInvTemplateNo": "1",
                "OrgInvSeries": `${inv_tplt}${inv_serial}`,
                "OrgInvDate": moment().format('YYYY-MM-DD'),
                "OrgInvNo": orgInvNo,
                //----Điều chỉnh/thay thế----
                "IsTaxReduction43": true,
                "RefID": orderNo,
        ...
        for await (let [index, item] of datas.entries()) {
            if (typeof item === "object") {
                var temp = item['TariffName'];
                var unit = await this.getUnitName(item['UnitCode']);
                var sz = item["IsoSizetype"] ? FunctionModel.getContSize(item["IsoSizetype"]) : (item["Size"] ? item["Size"] : '');
                if (sz != '') {
                    unit += " " + sz;
                }

                if (item.RemarkContainer && (item.RemarkContainer || '').split(',').length < 5) {
                    temp += '\n' + item.RemarkContainer;
                }
                else {
                    let bbl = (await this.getBBFromDraft(item.DraftNo));
                    temp += '\n' + (bbl || item.RemarkContainer);
                }
        ...
        this.data["serial"] = inv_serial;
        this.data["fkey"] = orderNo;
        this.data["inv"] = inv_serial + String(result[0]['InvNo']);
        this.data["invno"] = String(result[0]['InvNo']);
        this.data["hddt"] = 1; //them moi hd thu sau
        this.data['InvoiceTransId'] = result[0]['TransactionID']; //them moi hd thu sau
        // this.data["reservationCode"] = result["reservationCode"];
        this.data["invoiceDate"] = FunctionModel.moment().format('YYYY-MM-DD HH:mm:ss');
        return this.data;
    }

    //get pdf content (byte array)
    getInvView = async (req) => {
        if (!await this.getToken()) {
            this.data.success = false;
            this.data['warning_html'] = `<div style='width: 100vw;text-align: center;margin: -8px 0 0 -8px;font-weight: 600;font-size: 27px;color: white;background-color:#614040;line-height: 2;'>${this.data.error || 'Can not get token!'}</div>`;
            return this.data;
        }
        ...
        if (!this.data.success) {
            ...
            this.data['warning_html'] = `<div style='width: 100vw;text-align: center;margin: -8px 0 0 -8px;font-weight: 600;font-size: 27px;color: white;background-color:#614040;line-height: 2;'>
                ${this.data.error}
            </div>`;
            return this.data;
        }
        ...
        var path = this.config["API_PATH"] + `/code/itg/invoicepublished/downloadinvoice?downloadDataType=${fileType}`;
        ...
        try {
            ...
            this.data["content"] = fileType == 'xml' ? result[0]?.Data : Buffer.from(result[0]?.Data, "base64");
    ...
    viewDraftInv = async (req) => {
        ...
        if (this.responseData?.ErrorCode) {
            this.data.success = false;
            this.data.error = this.responseData?.Errors[0] || this.getErrorMisa(this.responseData?.ErrorCode);
            return this.data;
        }
